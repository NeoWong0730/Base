...
namespace Framework
{
    public struct BoneDampingSoloJob : IDisposableAnimationJob
    {
        //root用来同步[0]的Transform
        ...
        private void ComputeDampedPositions(AnimationStream stream)
        {
            ...
            var parentPosition = rootHandle.GetPosition(stream);
        ...
        private void ComputeJointLocalRotations(AnimationStream stream)
        {
            //var parentRotation = rootHandle.GetRotation(stream);
            var parentRotation = rootHandle.GetRotation(stream);
            for (var i = 1; i < jointHandles.Length - 2; ++i)
            {
