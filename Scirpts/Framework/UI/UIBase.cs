...
namespace Framework
{
    public class UIBase
    {
        ...
        public bool bLoad { get; private set; }
        ...
        public bool isVisible
        {
            get
            {
                return bLoad && gameObject != null && gameObject.activeSelf;
            }
        }
        ...
        private void MHandle_Completed(AsyncOperationHandle<GameObject> handle)
        {
            ...
            if (gameObject == null)
            {
                DebugUtil.LogErrorFormat("º”‘ÿUI‘§÷∆ÃÂ ß∞‹ {0}", nID);
                return;
            }
#if UNITY_EDITOR
...
#if UNITY_EDITOR
            ButtonDrawer buttonDrawer = gameObject.GetOrAddComponent<ButtonDrawer>();
#endif
            animatorControl = gameObject.GetComponent<UIAnimatorControl>();
            audioPlayer = gameObject.GetComponentsInChildren<UIController>(true);
                ...
                case EUIState.WaitShow:
                    {
                        if (!bLoad)
                        {
                            if (isLoadFinish)
                            {
                                ...
                                _DoLoaded();
                                DebugUtil.LogFormat(ELogType.eUIState, "{0} ProcessEvent(true)", sPrefabName);
                                ProcessEvents(true);
                            }
                        }
                        else
                        {
                            ...
                            fStateDuration -= dt;
                            if (fStateDuration <= 0 && bLoad && bStackHide == false)
                            {
                                _DoBeginEnter();
                            }
        ...
        protected virtual void _DoEndExit()
        {
            ...
            if (bLoad && gameObject.activeSelf)
            {
                OnHide();
        ...
        public virtual void OnSetData(object arg) { }
