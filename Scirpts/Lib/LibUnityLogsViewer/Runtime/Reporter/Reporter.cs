...
public class Reporter : MonoBehaviour
{
	...
	void DrawLogs()
	{
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			if (log == selectedLog) {
				...
				if (GUILayout.Button(log.condition + "\n" + log.stacktrace, logButtonStyle)) {
					//selectedIndex = startIndex + index ;
					selectedLog = log;
				}
				//GUILayout.FlexibleSpace();
	...
	private void OnGUI()
    {
		...
		{
			fScale = Screen.width / 1280f;
		}
		else
		{
			fScale = Screen.height / 720f;
		}
		Vector2 buttonSize = new Vector2(128, 64);

		if (mEnterWindowRect.size == Vector2.zero)
		{
			mEnterWindowRect = new Rect(Screen.width / 3f, buttonSize.y * fScale, buttonSize.x * fScale * 0.5f, buttonSize.y * fScale * 0.5f);
		}

		mEnterWindowRect = GUI.Window(2000, mEnterWindowRect, OnEnterWindow, string.Empty, GUI.skin.box);

		if (GUI.Button(new Rect(mEnterWindowRect.x, mEnterWindowRect.y + mEnterWindowRect.height, mEnterWindowRect.width, buttonSize.y * fScale), "LOG"))
        {
			doShow();
		}
    }
