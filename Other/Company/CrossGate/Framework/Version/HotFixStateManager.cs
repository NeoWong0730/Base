       ...
       /// <summary> 已经热更完毕的热更资源 </summary>
        ...
        public static void HotFixHttpErrorHitPoint(UnityWebRequestResult webRequestResult)
        {
            ...
            if (webRequestResult.Error == "Request timeout")//== 408
            {
                ...
                hashtable.Add("file_url", webRequestResult.Url);
                hashtable.Add("wait_time", "10");
                hashtable.Add("speed", string.Format("{0}/S", HotFixStateManager.CountSize(HotFixStateManager.Instance.DownloadSpeed))); //DownloadSpeed 不准
                HitPointManager.HitPoint("game_update_httptimeout", hashtable);
            }
            else if (webRequestResult.ResponseCode == 404)//文件丢失
            {
                Hashtable hashtable = new Hashtable();
                hashtable.Add("file_url", webRequestResult.Url);
                HitPointManager.HitPoint("game_update_missing", hashtable);
            }
            else//其他原因
            {
                Hashtable hashtable = new Hashtable();
                hashtable.Add("file_url", webRequestResult.Url);
                hashtable.Add("reason", webRequestResult.Error);
                HitPointManager.HitPoint("game_update_othercase", hashtable);
            }
