...
namespace Framework.Core.UI
{
    public class FUIBase
    {
        ...
        protected FUIStack mOwnerStack = null;
        ...
        public EUIState eState
        {
            ...
            private set
            {
                if (_eState != value)
                {
                    ...
                    DebugUtil.LogFormat(ELogType.eUIState, _eState != _eBefState, "{0} {1} -> {2}({3})", sPrefabName, _eBefState.ToString(), _eState.ToString(), _eAimState.ToString());
                }
            }
        }
        ...
        public void Init(int id, UIConfigData configData, FUIStack stack, bool isPreload)
        {
            mOwnerStack = stack;
            nID = id;
        ...
        public void SetSortingOrder(int order, bool force = false)
        {
            ...
            if (bLoaded)
            {
                ...
                mOwnerStack.eventEmitter.Trigger<uint, int>(FUIStack.EUIStackEvent.OnSortOrderChanged, mOwnerStack.stackID, nID);
        ...
        private void MHandle_Completed(AsyncOperationHandle<GameObject> handle)
        {
            ...
            if (gameObject.TryGetComponent<Canvas>(out Canvas tmpCanvas))
            {
                ...
                gameObject.GetNeedComponent<CanvasScreenMatch>();
                //移植状态更新中处理
                //SetSortingOrder(nSortingOrder, true);
            }

            GraphicRaycasterAdapter graphicRaycaster = gameObject.GetNeedComponent<GraphicRaycasterAdapter>();
            ...
            audioPlayer = gameObject.GetComponentsInChildren<UIAudio>(true);
            //TODO:预制默认设置为false
        ...
        protected virtual void _DoBeginEnter()
        {
            ...
            mOwnerStack.eventEmitter.Trigger<uint, int>(FUIStack.EUIStackEvent.BeginEnter, mOwnerStack.stackID, nID);
            mOwnerStack.nEventFlage |= (int)FUIStack.EUIStackEvent.BeginEnter;
        ...
        protected virtual void _DoEndEnter()
        {
            ...
            mOwnerStack.eventEmitter.Trigger<uint, int>(FUIStack.EUIStackEvent.EndEnter, mOwnerStack.stackID, nID);
            mOwnerStack.nEventFlage |= (int)FUIStack.EUIStackEvent.EndEnter;
        }
        protected virtual void _DoBeginExit()
        {
            ...
            mOwnerStack.eventEmitter.Trigger<uint, int>(FUIStack.EUIStackEvent.BeginExit, mOwnerStack.stackID, nID);
            mOwnerStack.nEventFlage |= (int)FUIStack.EUIStackEvent.BeginExit;
        }
        protected virtual void _DoEndExit()
        {
            ...
            mOwnerStack.eventEmitter.Trigger<uint, int>(FUIStack.EUIStackEvent.EndExit, mOwnerStack.stackID, nID);
            mOwnerStack.nEventFlage |= (int)FUIStack.EUIStackEvent.EndExit;
        }
