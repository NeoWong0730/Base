...
{
    [AddComponentMenu("UI/CP_ScrollRect", 37)]
    [SelectionBase]
    [ExecuteAlways]
    ...
    /// </remarks>
    public class CP_ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public bool beyondBorder = false;
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                ...
                if ((m_ContentBounds.size.x <= m_ViewBounds.size.x) || Mathf.Approximately(m_ContentBounds.size.x, m_ViewBounds.size.x))
                {
                    if (beyondBorder)
                    {
                        return (m_ViewBounds.min.x > m_ContentBounds.min.x) ? 999f : -999f;
                    }
                    else
                    {
        ...
        public float verticalNormalizedPosition
        {
            get
            {
                ...
                {
                    float diffY = m_ViewBounds.min.y - m_ContentBounds.min.y;
                    if (beyondBorder)
                    {
                        return (diffY > 0) ? 999f : -999f;
                    }
                    else
                    {
                        return (diffY > 0) ? 1f : 0f;
                    }
                }
