...
public class ScrollGridHorizontal : MonoBehaviour
{
    ...
    {
        RectTransform cellRect = cell.GetComponent<RectTransform>();
        int x = Mathf.Abs(Mathf.CeilToInt((-cellRect.anchoredPosition.x + cellWidth / 2) / cellWidth));
        int y = Mathf.CeilToInt(Mathf.Abs((cellRect.anchoredPosition.y) - cellHeight / 2) / cellHeight);
