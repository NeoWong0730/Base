...
public static partial class CameraManager
{
    ...
    public static Action onCameraVisibleChange;
    ...
    public static void World2UI(GameObject uigo, Vector3 worldPosition, Camera c3, Camera c2, float targetUiwidth, float targetUiheight, float xoffest, float yoffest)
    {
        if (uigo != null && c3 != null && c2 != null)
        {
            ...
            rectTransform.anchoredPosition = new Vector2(rectTransform.anchoredPosition.x * scale + xoffest,
                rectTransform.anchoredPosition.y * scale + yoffest);
        }
    }
    ...
    public static void CancelHide()
    {
        if (nHideCameraRef <= 0)
        {
            DebugUtil.LogErrorFormat("CancelHide 相机请求隐藏的次数已经为 {0}, 检测调用逻辑是否错误", nHideCameraRef);
            nHideCameraRef = 0;
        }
        else
        {
    ...
    public static void CancelReduceMainCameraQuality()
    {
        ...
        {
            DebugUtil.LogErrorFormat("CancelReduceMainCameraQuality 引用次数已经为 {0}, 检测调用逻辑是否错误", nReduceMainCameraQualityRef);
    ...
    private static void RefreshShadow()
    {
        if (mCamera)
        {
            UnityEngine.Rendering.Universal.UniversalAdditionalCameraData additionalCameraData = mCamera.GetComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
            if (!additionalCameraData)
            {
                additionalCameraData = mCamera.gameObject.AddComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
            }
    ...
    public static void SetMainCameraEnable(bool enable)
    {
        if (mCamera != null)
        {
            ...
            if (onCameraVisibleChange != null)
                onCameraVisibleChange();
            //             mCamera.enabled = enable;
            // 
            //             UniversalAdditionalCameraData cameraData = mCamera.GetUniversalAdditionalCameraData();
            //             if (cameraData)
            //             {
            ...
            //                 {
            //                     UniversalAdditionalCameraData stackCameraData = cameras[i].GetUniversalAdditionalCameraData();
            //                     if (stackCameraData)
            //                     {
            //                         stackCameraData.renderType = enable ? CameraRenderType.Overlay : CameraRenderType.Base;
            //                     }
            //                 }
            //             }
        }
