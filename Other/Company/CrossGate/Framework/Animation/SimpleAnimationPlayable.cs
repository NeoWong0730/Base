...
namespace Framework
{
    public partial class SimpleAnimationPlayable : PlayableBehaviour
    {
        ...
        private StateInfo DoAddClip(string name, AnimationClip clip)
        {
            ...
            if (m_Mixer.IsValid() && index == m_Mixer.GetInputCount())
            {
                m_Mixer.SetInputCount(index + 1);
            }

            if (self.IsValid() && graph.IsValid())
            {
        ...
        public void SetSpeed(float speed)
        {
            for (int i = 0; i < m_States.Count; i++)
            {
                m_States.SetStateSpeed(i, speed);
            }
        }
        ...
        public int Pause(string name)
        {
            ...
            if (state == null)
            {
                Debug.LogError(string.Format("Cannot queue Pause to state with name {0} because there is no state with that name", name));
                return -1;
            }
        ...
        public int RemovePause(string name)
        {
            StateInfo state = m_States.FindState(name);
            if (state == null)
            {
                Debug.LogError(string.Format("Cannot queue RemovePause to state with name {0} because there is no state with that name", name));
                return -1;
            }
        ...
        private void Crossfade(int index, float time)
        {
            for (int i = 0; i < m_States.Count; i++)
            {
                StateInfo state = m_States[i];
        ...
        public int Crossfade(string name, float time)
        {
            StateInfo state = m_States.FindState(name);
            if (state == null)
            {
        ...
        public bool CrossfadeQueued(int index, float time, QueueMode queueMode)
        {
            StateInfo newState = CloneState(index);
