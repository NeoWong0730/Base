...
namespace Logic
{
    public class UI_Trade_Panel_Publicity_View : UI_Trade_Panel_Publicity_View_Top.IListner, UI_Trade_Panel_Publicity_View_Bottom.IListner
    {
        private Transform transform;

        private GameObject m_Empty;
        private UI_Trade_Panel_Publicity_View_Top m_Top;
        private UI_Trade_Panel_Publicity_View_Item m_ViewItem;
        private UI_Trade_Panel_Publicity_View_Bottom m_Bottom;
        ...
        public void Init(Transform trans)
        {
            ...
            m_Top = new UI_Trade_Panel_Publicity_View_Top();
            m_Top.Init(transform.Find("GameObject/View_Top"));
            m_Top.Register(this);

            m_ViewItem = new UI_Trade_Panel_Publicity_View_Item();
            m_ViewItem.Init(transform.Find("GameObject/Rect"));

            m_Bottom = new UI_Trade_Panel_Publicity_View_Bottom();
        ...
        public void ProcessEvents(bool register)
        {
            ...
            Sys_Trade.Instance.eventEmitter.Handle(Sys_Trade.EEvents.OnOfferPriceNtf, OnOfferPriceNtf, register);
        ...
        {
            if (Sys_Trade.Instance.CurPageType == Sys_Trade.PageType.Publicity)
            {
        ...
        private void OnListReq(uint curPage = 1u)
        {
            if (Sys_Trade.Instance.SearchParam.isSearch && Sys_Trade.Instance.SearchParam.showType == TradeShowType.Publicity)
            {
                ...
                bool bCross = Sys_Trade.Instance.CurPublicityServerType == Sys_Trade.ServerType.Cross;
                ulong goodsUId = Sys_Trade.Instance.SearchParam.goodsUId;

                Sys_Trade.Instance.OnTradeListReq(bCross, (uint)TradeShowType.Publicity, m_Top.IsDownRank, curPage, searchType, infoId, Sys_Trade.Instance.CurPublicitySubCategory, m_Top.SubClassId, equipParam, petParam, petEquipParam, oraParam, goodsUId);

                //Sys_Trade.Instance.SearchParam.Reset();
            }
            else
            {
                //TradeShowType showType = m_Top.IsBargin ? TradeShowType.Discuss : TradeShowType.OnSale;
                bool bCross = Sys_Trade.Instance.CurPublicityServerType == Sys_Trade.ServerType.Cross;
                Sys_Trade.Instance.OnTradeListReq(bCross, (uint)TradeShowType.Publicity, m_Top.IsDownRank, curPage, TradeSearchType.Category, 0u, Sys_Trade.Instance.CurPublicitySubCategory, m_Top.SubClassId);
            }
        }
