...
namespace Logic
{
    public class UI_Trade_Panel_Buy_SubCategory
    {
        private class SubCategory_Cell
        {
            ...
            {
                Sys_Trade.Instance.SetBuySubCategory(m_SubCategoryId);
            }
        ...
        public void ProcessEvents(bool register)
        {
            ...
            Sys_Trade.Instance.eventEmitter.Handle(Sys_Trade.EEvents.OnViewBuyListSaleCountNtf, OnListSaleCountNtf, register);
        }
        ...
        public void UpdateInfo()
        {
            ...
            Sys_Trade.Instance.OnSaleCountReq(Sys_Trade.Instance.CurBuyServerType == Sys_Trade.ServerType.Cross,
                     Sys_Trade.Instance.CurBuyCategory, false);
        }
        ...
        {
            CSVCommodityList.Data data = CSVCommodityList.Instance.GetConfData(Sys_Trade.Instance.CurBuyCategory);
            if (data != null)
            ...
            if (Sys_Trade.Instance.SearchParam.isSearch && Sys_Trade.Instance.SearchParam.showType != TradeShowType.Publicity)
            {
                ...
                if (isContains)
                {
                    Sys_Trade.Instance.SetBuySubCategory(Sys_Trade.Instance.SearchParam.SubCategory);
                }
                else
                {
                    if (Sys_Trade.Instance.SearchParam.SubCategory != 0u)
            ...
            {
                if (Sys_Trade.Instance.CurBuySubCategory != 0)
                {
                    CSVCommodityCategory.Data temp = CSVCommodityCategory.Instance.GetConfData(Sys_Trade.Instance.CurBuySubCategory);
                    if (temp.list != Sys_Trade.Instance.CurBuyCategory)
                        Sys_Trade.Instance.CurBuySubCategory = 0;
                    else
                        Sys_Trade.Instance.SetBuySubCategory(Sys_Trade.Instance.CurBuySubCategory);
                }
            }
        }
    }
