...
namespace Logic
{
    public class UI_FirstCharge : UIBase
    {
        ...
        private void Parse()
        {
            ...
            btnBuy = transform.Find("Animator/View_Right/Btn_01").GetComponent<Button>();
            btnBuy.onClick.AddListener(OnBtnBuyClick);
            txtbtnBuy = transform.Find("Animator/View_Right/Btn_01/Text").GetComponent<Text>();
        ...
        private void UpdateBtnState()
        {
            ...
            ImageHelper.SetImageGray(btnBuy.transform.GetComponent<Image>(), false, true);
            if (isCharge)
            {
                ...
                switch (state)
                {
                    case (uint)ChargeRewardStatus.None:
                        if(selectDayNum == 2)
                        {
                            txtbtnBuy.text = LanguageHelper.GetTextContent(11745);//第二天领取
                        }
                        else if(selectDayNum == 3)
                        {
                            txtbtnBuy.text = LanguageHelper.GetTextContent(11746);//第三天领取
                        }
                        break;
                    case (uint)ChargeRewardStatus.CanReceive:
                        txtbtnBuy.text = LanguageHelper.GetTextContent(11744);//领取
                        break;
                    case (uint)ChargeRewardStatus.Receivid:
                        txtbtnBuy.text = LanguageHelper.GetTextContent(4725);//已领取
                        ImageHelper.SetImageGray(btnBuy.transform.GetComponent<Image>(), true, true);
                        break;
                }
            }
            else
            {
                txtbtnBuy.text = LanguageHelper.GetTextContent(4903);//前往充值
            }
        }
        ...
        private void OnBtnBuyClick()
        {
            if (Sys_OperationalActivity.Instance.IsFirstCharge)
            {
                ...
                if(state == (uint)ChargeRewardStatus.CanReceive)
                {
                    ...
                    UIManager.HitButton(EUIID.UI_FirstCharge, "GetReward_" + selectDayNum.ToString());
                }
            }
            else
            {
                ...
                UIManager.HitButton(EUIID.UI_FirstCharge, "GoChargeMall");
            }
        }
