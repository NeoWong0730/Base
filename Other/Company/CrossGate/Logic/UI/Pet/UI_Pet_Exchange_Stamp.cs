...
namespace Logic
{
    public class UI_Pet_Exchange_Stamp_Layout
    {
    ...
    public class UI_Pet_Exchange_StampLeftView : UIComponent
    {
    ...
    public class UI_Pet_Exchange_StampRightView : UIComponent
    {
        public class UI_PetExchangeGetItem : UIComponent
        {
            ...
            public override void SetData(params object[] arg)
            {
                ...
                {
                    List<uint> data = arg[0] as List<uint>;
                    uint itemId = data[0];
                    uint count = data[1];
                    float itemNums = data[2] / Convert.ToSingle(arg[1]);
                    propItem.SetData(new MessageBoxEvt(EUIID.UI_SelectItem, new PropIconLoader.ShowItemData(itemId, count, false, false, false, false, false, false, false,true)));
                    ...
                    TextHelper.SetText(itemLvText,60664, count.ToString());
        ...
        private void ExchangeBtnClicked()
        {
            if (clientPet.petUnit.SimpleInfo.Bind)
            {
                Sys_Hint.Instance.PushContent_Normal(LanguageHelper.GetTextContent(12475));
                return;
            }
            //唯一一只无法兑换
            else if (Sys_Pet.Instance.IsUniquePet(clientPet.petData.id))
            {
            ...
            //非可兑换
            else if (!clientPet.petData.is_gold_charge)
            {
                Sys_Hint.Instance.PushContent_Normal(LanguageHelper.GetTextContent(12474));
                return;
            }
            //出战宠物
            else if (Sys_Pet.Instance.fightPet.IsSamePet(clientPet.GetPetUid()))
            ...
            {
                Sys_Hint.Instance.PushContent_Normal(LanguageHelper.GetTextContent(680002041));
                return;
            }
            ...
            PromptBoxParameter.Instance.Clear();
            PromptBoxParameter.Instance.content = LanguageHelper.GetTextContent(12473, clientPet.GetPetNmae());
            PromptBoxParameter.Instance.SetConfirm(true, () =>
            {

                Sys_Pet.Instance.PetStampExchangeReq(clientPet.GetPetUid());
            });
            PromptBoxParameter.Instance.SetCancel(true, null);
            UIManager.OpenUI(EUIID.UI_PromptBox, false, PromptBoxParameter.Instance);
        }
        ...
        protected override void Refresh()
        {
            if(null != clientPet)
            {
                ...
                if (canCharge)
                {
                    itemAndWeight = Sys_Pet.Instance.GetExchangeStampItemData(clientPet.petUnit.SimpleInfo.Score, out weight);
                    if(null != itemAndWeight)
                    {
                        targetCount = itemAndWeight.Count;
                    }
                    else
                    {
                        targetCount = 0;
                ...
                if (!canCharge)
                {
                    TextHelper.SetText(conditonTipsText, 12474);
                }

                conditonTipsText.gameObject.SetActive(!canCharge);
        ...
        {
            item.SetData(itemAndWeight[index], weight);
        }
    ...
    public class UI_Pet_Exchange_Stamp : UIBase, UI_Pet_Exchange_Stamp_Layout.IListener
    {
        private UI_Pet_Exchange_Stamp_Layout layout = new UI_Pet_Exchange_Stamp_Layout();

        private UI_CurrencyTitle UI_CurrencyTitle;

        public UI_Pet_Exchange_StampLeftView leftview;
        public UI_Pet_Exchange_StampRightView rightview;
        ...
        protected override void OnLoaded()
        {
            ...
            rightview = AddComponent<UI_Pet_Exchange_StampRightView>(layout.rightViewGo.transform);
            leftview = AddComponent<UI_Pet_Exchange_StampLeftView>(layout.leftViewGo.transform);
        ...
        public void OncloseBtnClicked()
        {
            ...
            UIManager.CloseUI(EUIID.UI_Pet_Exchange_Stamp);
        }
        #endregion
    }
}