...
namespace Logic
{
    public class UI_Ornament_ViewRight : UIComponent
    {
        #region 界面组件
        private ViewRightCell cellUpgrade;
        private ViewRightCell cellRecast;
        #endregion
        ...
        private void Init()
        {
            cellUpgrade = AddComponent<ViewRightCell>(transform.Find("Label_Scroll01/TabList/TabItem01"));
            cellUpgrade.PageType = (uint)EOrnamentPageType.Upgrade;
            cellUpgrade.Register(OnTabSelect);
            dicCells.Add((uint)cellUpgrade.PageType, cellUpgrade);

            cellRecast = AddComponent<ViewRightCell>(transform.Find("Label_Scroll01/TabList/TabItem02"));
            cellRecast.PageType = (uint)EOrnamentPageType.Recast;
