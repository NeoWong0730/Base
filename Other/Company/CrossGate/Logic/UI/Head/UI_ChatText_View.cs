...
namespace Logic
{
    public class UI_ChatText_View : UIComponent
    {
        private GameObject itemGo;
        public Text collectNum;

        private List<uint> chatWords = new List<uint>();
        ...
        protected override void Loaded()
        {
            ...
            chatWords = Sys_Head.Instance.GetDatasId(EHeadViewType.ChatTextView);
        ...
        public override void Show()
        {
            ...
            infinityCount = chatWords.Count;
            infinityGrid.CellCount = infinityCount;
            infinityGrid.ForceRefreshActiveCell();
            collectNum.text = LanguageHelper.GetTextContent(10884, Sys_Head.Instance.GetTypesActiveCount(EHeadViewType.ChatTextView).ToString(), chatWords.Count.ToString());
        ...
        {
            uint id = chatWords[index];
            UI_ChatFrame_Item item = cell.mUserData as UI_ChatFrame_Item;
            item.SetData(id, EHeadViewType.ChatTextView);
            items.Add(item);
            if (index == 0)
            {
            ...
            if (clientHeadData.chatTextId == id)
            {
                item.select.SetActive(true);
        ...
        private void OnUsingUpdate()
        {
            for (int i = 0; i < items.Count; ++i)
            {
                items[i].useTag.SetActive(items[i].id == clientHeadData.chatTextId);
            }
        }
