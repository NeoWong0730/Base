...
namespace Logic
{
    public class UI_Society_FriendGroupCreate_Layout
    {
        public class RoleInfoItem
        {
            ...
            void OnClickToggle(bool isOn)
            {
                if (isOn)
                {
                    UI_Society_FriendGroupCreate.selectRoleInfos[roleInfo.roleID] = roleInfo;
                }
                else
                {
                    if (UI_Society_FriendGroupCreate.selectRoleInfos.ContainsKey(roleInfo.roleID))
                    {
                        UI_Society_FriendGroupCreate.selectRoleInfos.Remove(roleInfo.roleID);
                    }
                }
                toggleChange?.Invoke();
            }
    ...
    public class UI_Society_FriendGroupCreate : UIBase, UI_Society_FriendGroupCreate_Layout.IListener
    {
        private UI_Society_FriendGroupCreate_Layout layout = new UI_Society_FriendGroupCreate_Layout();
        ...
        void UpdateRoleList(Dictionary<ulong, Sys_Society.RoleInfo> _roleInfos)
        {
            ...
            foreach (var roleInfo in roleInfos.Values)
            {
                ...
                UI_Society_FriendGroupCreate_Layout.RoleInfoItem roleInfoItem = new UI_Society_FriendGroupCreate_Layout.RoleInfoItem(roleItemGo);
        ...
        {
            UIManager.CloseUI(EUIID.UI_Society_FriendGroupCreate);
        }
        ...
        public void OnClickSureButton()
        {
            ...
            Sys_Society.Instance.ReqFriendGroup(layout.friendgroupNameInput.text, string.Empty, roleIDs, true);
        }
