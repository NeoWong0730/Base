...
namespace Logic
{
    public partial class UI_Activity_Exchange_HeFu : UIBase
    {
        #region GUILable
        ...
        private void SetTitleAndBG()
        {
            if (Sys_ItemExChange.Instance.isHeFuActivity)
            {
                TextHelper.SetText(m_AcTitle, Sys_ItemExChange.Instance.curHeFuCSVActivityRulerData.Activity_Name);
                ImageHelper.SetIcon(m_AcBG, null, Sys_ItemExChange.Instance.curHeFuCSVActivityRulerData.Back_Image, false);
                ImageHelper.SetTexture(m_AcIcon, Sys_ItemExChange.Instance.curHeFuCSVActivityRulerData.Foreg_Image, true);
            }
            else if (Sys_ActivityQuest.Instance.isHeFuActivity)
            {
                TextHelper.SetText(m_AcTitle, Sys_ActivityQuest.Instance.curCSVActivityRulerDataHeFu.Activity_Name);
                ImageHelper.SetIcon(m_AcBG, null, Sys_ActivityQuest.Instance.curCSVActivityRulerDataHeFu.Back_Image, false);
                ImageHelper.SetTexture(m_AcIcon, Sys_ActivityQuest.Instance.curCSVActivityRulerDataHeFu.Foreg_Image, true);
            }
        }

        private void RefreshRedPoint()
        {
            m_ItemExRed.SetActive(Sys_ItemExChange.Instance.hasHeFuRed());
            m_QuestRed.SetActive(Sys_ActivityQuest.Instance.hasHeFuRed());
        }
        ...
        {
            if (!Sys_ItemExChange.Instance.isHeFuActivity || !Sys_ActivityQuest.Instance.isHeFuActivity)//任意一个活动没开启 就把两个页签隐藏掉
            {
        ...
        private void RefreshItemEntry()
        {
            if (m_CurLable == 1)
            {
                if (Sys_ItemExChange.Instance.isHeFuActivity)
                {
                    Sys_ItemExChange.Instance.UpdateHeFuExItemState();
                    m_InfinityGrid_ExItem.CellCount = Sys_ItemExChange.Instance.exHeFuDatas.Count;
                    m_InfinityGrid_ExItem.ForceRefreshActiveCell();
                }
            ...
            else if (m_CurLable == 2)
            {
                ...
                {
                    m_InfinityGrid_QtTask.CellCount = Sys_ActivityQuest.Instance.questHeFuDatas.Count;
                    m_InfinityGrid_QtTask.ForceRefreshActiveCell();
                }

                m_DaliyTips2.SetActive(Sys_ActivityQuest.Instance.isHeFuActivity);
            }
        }
        ...
        private void SetActivityValidTime()
        {
            if (m_CurLable == 1)
            {
                DateTime startTime = TimeManager.GetDateTime(Sys_ItemExChange.Instance.startHeFuTime);
                DateTime endTime = TimeManager.GetDateTime(Sys_ItemExChange.Instance.endHeFuTime);
                ...
                if (Sys_ItemExChange.Instance.endHeFuTime < Sys_Time.Instance.GetServerTime())
                {
                    TextHelper.SetText(m_RemainTime, 2025733);
                }
                else
                {
                    uint remainTime = Sys_ItemExChange.Instance.endHeFuTime - Sys_Time.Instance.GetServerTime();
                    TextHelper.SetText(m_RemainTime, LanguageHelper.TimeToString(remainTime, LanguageHelper.TimeFormat.Type_12));
                }
            }
            else if (m_CurLable == 2)
            {
                DateTime startTime = TimeManager.GetDateTime(Sys_ActivityQuest.Instance.startHeFuTime);
                DateTime endTime = TimeManager.GetDateTime(Sys_ActivityQuest.Instance.endHeFuTime);
                TextHelper.SetText(m_ActivityValidTime, LanguageHelper.GetTextContent(591000705, startTime.Year.ToString(), startTime.Month.ToString(), startTime.Day.ToString()
                   , GetTimeFormat(startTime.Hour), GetTimeFormat(startTime.Minute), endTime.Year.ToString(), endTime.Month.ToString(), endTime.Day.ToString()
                   , GetTimeFormat(endTime.Hour), GetTimeFormat(endTime.Minute)));

                if (Sys_ActivityQuest.Instance.endHeFuTime < Sys_Time.Instance.GetServerTime())
                {
                    TextHelper.SetText(m_RemainTime, 2025733);
                }
                else
                {
                    uint remainTime = Sys_ActivityQuest.Instance.endHeFuTime - Sys_Time.Instance.GetServerTime();
                    TextHelper.SetText(m_RemainTime, LanguageHelper.TimeToString(remainTime, LanguageHelper.TimeFormat.Type_12));
                }
            }
        }
        ...
        private void OnCellChange_Ex(InfinityGridCell cell, int index)
        {
            ...
            grid.SetData(Sys_ItemExChange.Instance.exHeFuDatas[index].id);
        }
        ...
        public class ExItemEntry
        {
            ...
            public void SetData(uint id)
            {
                ...
                exData = Sys_ItemExChange.Instance.TryGetExHeFuData(id);
            ...
            private void OnExButtonClicked()
            {
                ...
                if (m_Data.Limit_Type == 0)//没有限制
                {
                    ...
                    {
                        //兑换
                        Sys_ItemExChange.Instance.ActivityHeFuExchangeTakeReq(m_Id);
                    }
                    else
                    {
            ...
            {
                exData.openRed = arg;
                Sys_ItemExChange.Instance.SaveHeFuMemory();
            }
        }
    }
}
