    ...
    public class SystemModuleManager
    {
...
#if DEBUG_MODE
            Lib.Core.DebugUtil.LogTimeCost(Lib.Core.ELogType.eNone, "SystemModuleRegister.Unregister()", ref timePoint);
#endif
        }
...
#if DEBUG_MODE
                    if (Lib.Core.DebugUtil.IsOpenLogType((int)Lib.Core.ELogType.eExecuteTime))
                    {
                        Lib.Core.DebugUtil.LogTimeCost(Lib.Core.ELogType.eExecuteTime, string.Format("{0}.Init()", _systemModels[_initIndex].GetType().ToString()), ref timePoint, 10);
                    }
#endif
                }
                else
        ...
        public static void OnLogin()
        {
            ...
            for (int i = _systemModels.Count - 1; i >= 0; --i)
            {
                ...
                if (Lib.Core.DebugUtil.IsOpenLogType((int)Lib.Core.ELogType.eExecuteTime))
                {
                    Lib.Core.DebugUtil.LogTimeCost(Lib.Core.ELogType.eExecuteTime, string.Format("{0}.OnLogin()", _systemModels[i].GetType().ToString()), ref timePoint, 2);
                }                    
#endif
            }
        }
...
#if DEBUG_MODE
                if (Lib.Core.DebugUtil.IsOpenLogType((int)Lib.Core.ELogType.eExecuteTime))
                    Lib.Core.DebugUtil.LogTimeCost(Lib.Core.ELogType.eExecuteTime, string.Format("{0}.OnLogout()", _systemModels[i].GetType().ToString()), ref timePoint, 2);
#endif
            }
        }
...
#if DEBUG_MODE
                if (Lib.Core.DebugUtil.IsOpenLogType((int)Lib.Core.ELogType.eExecuteTime))
                    Lib.Core.DebugUtil.LogTimeCost(Lib.Core.ELogType.eExecuteTime, string.Format("{0}.OnSyncFinished()", _systemModels[i].GetType().ToString()), ref timePoint, 2);
#endif
            }
        }
