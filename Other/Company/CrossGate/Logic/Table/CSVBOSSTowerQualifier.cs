...
namespace Table
{
	...
	sealed public partial class CSVBOSSTowerQualifier : Framework.Table.TableBase<CSVBOSSTowerQualifier.Data>
	{
	    sealed public class Data
	    {
			...
			public readonly uint floor_number;
            ...
            public Data(uint id, BinaryReader binaryReader, TableShareData shareData)
            {
				...
				floor_number = ReadHelper.ReadUInt(binaryReader);
		...
		{
			return "Config/Table/CSVBOSSTowerQualifier.bytes";
		}

		private static CSVBOSSTowerQualifier instance = null;			
		public static CSVBOSSTowerQualifier Instance
		{
			get
			{
#if DEBUG_MODE
				if (instance == null)
				{
					DebugUtil.LogError("配置表CSVBOSSTowerQualifier 尚未构建实例, 请添加配置表加载到 CSVRegister");
				}
#endif
				return instance;
			}
        ...
        {
			DebugUtil.LogFormat(ELogType.eTable, "加载CSVBOSSTowerQualifier forceReload = {0}", forceReload);

            if (instance == null)
            {
                instance = new CSVBOSSTowerQualifier();
        ...
        {
            DebugUtil.Log(ELogType.eTable, "卸载CSVBOSSTowerQualifier");
    ...
    sealed public partial class CSVBOSSTowerQualifier : FCSVBOSSTowerQualifier
    {
        public static string ConfigPath()
		{
			return "Config/Table/CSVBOSSTowerQualifier.bytes";
