    ...
    private void OnEnable()
    {
        ...
        for (int i = 0; i < mRenderDatas.Length; ++i)
        {
            ...
            for (int j = 0; j < texNameIDs.Length; ++j)
            {
                ...
                if (!texture)
                {
                    Lib.Core.DebugUtil.LogFormat(Lib.Core.ELogType.eScene, "{0} is not has texture {1}", renderData.material.name, texNames[j]);
                    continue;
                }
#endif
                ...
                if (!texture2D)
                {
                    Lib.Core.DebugUtil.LogFormat(Lib.Core.ELogType.eScene, "{0} is not texture2D", texture.name);
                    continue;
                }
