...
public class BuildWindow : EditorWindow
{
    ...
    private void OnGUI_Base()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        if (string.IsNullOrWhiteSpace(mBuildSetting.sAssetVersion))
        {
            mBuildSetting.sAssetVersion = "0.0.0";
        }
        mVersionSetting.AssetVersion = mBuildSetting.sAssetVersion = EditorGUILayout.TextField("资源版本号", mBuildSetting.sAssetVersion, GUILayout.Width(300));
        EditorGUILayout.EndHorizontal();
    ...
    private void OnGUI_Build()
    {
        ...
        {
            mBuildSetting.bUseSplitPack = EditorGUILayout.Toggle("使用分包策略", mBuildSetting.bUseSplitPack);
...
//#endif
            ...
            mBuildSetting.eRuntimeMode = ERuntimeMode.Normal;
            EditorGUILayout.LabelField("运行时模式", "Normal");
...
//#endif
    ...
    private void OnGUI_AssetSetting()
    {
        ...
        if (GUILayout.Button("生成 AaGroup(分析+修正)", GUILayout.Width(180)))
        {
            ...
            {
                AddressableBundleGroupSetting.GenLocalAssetGroup_Addressable(mBuildSetting, true);
            }
            GUIUtility.ExitGUI();
        }
        ...
        if (GUILayout.Button("生成 AaGroup(No 分析+修正)", GUILayout.Width(180)))
        {
            ...
            {
                AddressableBundleGroupSetting.GenLocalAssetGroup_Addressable(mBuildSetting, false);
            }
            GUIUtility.ExitGUI();
        }
    ...
    private void GenLocalBundle(string mLocalBuildPath)
    {
        ...
        AddressableBundleGroupSetting.GenLocalAsset_Addressable(mBuildSetting);
