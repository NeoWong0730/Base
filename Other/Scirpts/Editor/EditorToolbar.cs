...
[InitializeOnLoad]
public static class ToolbarExtander {
...
public class ToolbarGUI {
    ...
    static ToolbarGUI() {
        ToolbarExtander.LeftToolbarGUI.Add(OnLeftToolbarGUI);
        ToolbarExtander.RightToolbarGUI.Add(OnRightToolbarGUI);
    }
    ...
    static void OnLeftToolbarGUI() {
        ...
        if (GUILayout.Button(ToolbarStyles.SaveFromPlayButtonContent, ToolbarStyles.CommandButtonStyle)) {
            ...
            // buildsettings中有则加载第一个，否则手动指定地址
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) {
                ...
                if (EditorBuildSettings.scenes.Length > 0) {
                    ...
                    // 比如："Assets/ResourcesAB/GameStart.unity";
                    string path = "主场景确切路径";
                    EditorSceneManager.OpenScene(path);
                }
            }
        ...
        //     if (Application.isPlaying || EditorApplication.isPaused || EditorApplication.isCompiling) {
        //         Debug.LogError("You cant do it when playing/pausing/compiling");
        //         return;
        //     }
        //
        //     if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) {
    ...
    static void OnRightToolbarGUI() {
        ...
        if (Application.isPlaying) {
            // 非运行时不能修改，因为会修改到TimeManager.asset,防止误改
