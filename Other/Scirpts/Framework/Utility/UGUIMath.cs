...
namespace Framework
{
	...
	static public class NGUIMath
	{
		...
		static public Color IntToColor(int val)
		{
			...
			Color c = Color.black;
			c.r = factor * ((val >> 24) & 0xFF);
			c.g = factor * ((val >> 16) & 0xFF);
			c.b = factor * ((val >> 8) & 0xFF);
			c.a = factor * (val & 0xFF);
			return c;
		}
		...
		// 获取两个点的斜率角度
		public static float GetAngleByPoints(Vector2 left, Vector2 right)
		{
			...
			if (delta.x > 0)
			{
				if (delta.y > 0)
				{
					return Vector2.Angle(delta, new Vector2(1f, 0f));
				}
				else
				{
					return -Vector2.Angle(delta, new Vector2(1f, 0f));
			...
			else
			{
				if (delta.y > 0)
				{
					return 180 - Vector2.Angle(delta, new Vector2(-1f, 0f));
				}
				else
				{
					return 180 + Vector2.Angle(delta, new Vector2(-1f, 0f));
		...
		/// <summary>
		/// f为真数 g为底数   如 Mathf.Log(16, 2)=4
		/// </summary>
		/// <param name="f"></param>
		/// <param name="g"></param>
		/// <returns></returns>
		public static float GetLogarithmic(float f, float g)
		{
			return Mathf.Log(f, g);
		}

		public static void GetRemainTime(uint time, out uint day, out uint hour, out uint minute, out uint second)
		{
			day = time / 86400;
