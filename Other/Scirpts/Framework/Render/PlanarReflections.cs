...
namespace UnityEngine.Rendering.Universal
{
    ...
    {
        public static bool bForceClose = false;
        ...
        private void UpdateReflectionCamera(Camera realCamera)
        {
            ...
            m_ReflectionCamera.rect = new Rect(0, 0, 1, 1);
        ...
        {
            GameObject go =
                new GameObject($"Refl Camera id{GetInstanceID().ToString()} for {currentCamera.name} {currentCamera.GetInstanceID().ToString()}",
                    typeof(Camera));
            ...
            lwrpCamData.renderShadows = false;// true; // turn off shadows for the reflection camera
        ...
        public void RenderPipelineManager_beginCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            ...
            bool needHDR = camera.allowHDR && UnityEngine.Rendering.Universal.UniversalRenderPipeline.asset.supportsHDR;

            var res = ReflectionResolution(camera, UnityEngine.Rendering.Universal.UniversalRenderPipeline.asset.renderScale);
            RenderTextureFormat hdrFormat = RenderTextureFormat.Default;

            if (needHDR)
            {
                bool useHDR10 = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGB111110Float);
                hdrFormat = useHDR10 ? RenderTextureFormat.RGB111110Float : RenderTextureFormat.DefaultHDR;
            }
            ...
            m_ReflectionTexture = RenderTexture.GetTemporary(res.x, res.y, 16, GraphicsFormatUtility.GetGraphicsFormat(hdrFormat, true));
