...
namespace NWFramework
{
    public static partial class Utility
    {
        ...
        public static class Encryption
        {
            ...
            {
                return GetXorBytes(bytes, 1, QuickEncryptLength, code);
            }
            ...
            public static void GetSelfXorBytes(byte[] bytes, int startIndex, int length, byte[] code)
            {
                ...
                if (code == null)
                {
                    throw new NWFrameworkException("Code is invalid.");
                }

                int codeLength = code.Length;
                if (codeLength <= 0)
                {
                    throw new NWFrameworkException("Code length is invalid.");
                }

                if (startIndex < 0 || length < 0 || startIndex + length > bytes.Length)
                {
                    throw new NWFrameworkException("Start index or length is invalid.");
                }
