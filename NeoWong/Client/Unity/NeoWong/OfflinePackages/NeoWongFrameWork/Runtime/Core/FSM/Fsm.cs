...
namespace NWFramework
{
    internal sealed class Fsm<T> : FsmBase, IMemory, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            if (owner == null)
            {
                throw new NWFrameworkException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new NWFrameworkException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new NWFrameworkException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new NWFrameworkException("FSM is running, can not start again.");
            }

            FsmState<T> state = GetState<TState>();
            if (state == null)
            {
                throw new NWFrameworkException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), typeof(TState).FullName));
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
                throw new NWFrameworkException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new NWFrameworkException(Utility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new NWFrameworkException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
        ...
        public void GetAllStates(List<FsmState<T>> results)
        {
            if (results == null)
            {
                throw new NWFrameworkException("Results is invalid.");
            }
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new NWFrameworkException("Data name is invalid.");
            }
        ...
        internal void ChangeState(Type stateType)
        {
            if (_currentState == null)
            {
                throw new NWFrameworkException("Current state is invalid.");
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new NWFrameworkException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
