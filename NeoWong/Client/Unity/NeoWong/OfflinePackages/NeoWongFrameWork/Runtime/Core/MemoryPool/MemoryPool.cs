...
namespace NWFramework
{
    ...
    public static partial class MemoryPool
    {
        ...
        public static void Release(IMemory memory)
        {
            if (memory == null)
            {
                throw new NWFrameworkException("Memory is invalid.");
            }
        ...
        private static void InternalCheckMemoryType(Type memoryType)
        {
            ...
            if (memoryType == null)
            {
                throw new NWFrameworkException("Memory type is invalid.");
            }

            if (!memoryType.IsClass || memoryType.IsAbstract)
            {
                throw new NWFrameworkException("Memory type is not a non-abstract class type.");
            }

            if (!typeof(IMemory).IsAssignableFrom(memoryType))
            {
                throw new NWFrameworkException(Utility.Text.Format("Memory type '{0}' is invalid.", memoryType.FullName));
            }
        }
        ...
        /// </summary>
        /// <param name="memoryType">内存对象类型</param>
        /// <returns>内存池收集器</returns>
        private static MemoryCollection GetMemoryCollection(Type memoryType)
        {
            if (memoryType == null)
            {
                throw new NWFrameworkException("MemoryType is invalid.");
            }
