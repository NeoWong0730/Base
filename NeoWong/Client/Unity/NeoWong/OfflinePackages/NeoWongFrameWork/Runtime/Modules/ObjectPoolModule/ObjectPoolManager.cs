...
namespace NWFramework
{
    ...
    internal sealed partial class ObjectPoolManager : ModuleImp, IObjectPoolManager
    {
        ...
        public bool HasObjectPool(Type objectType)
        {
            if (objectType == null)
            {
                throw new NWFrameworkException("Object type is invalid.");
            }

            if (!typeof(ObjectBase).IsAssignableFrom(objectType))
            {
                throw new NWFrameworkException(Utility.Text.Format("Object type '{0}' is invalid.", objectType.FullName));
            }
        ...
        public bool HasObjectPool(Predicate<ObjectPoolBase> condition)
        {
            if (condition == null)
            {
                throw new NWFrameworkException("Condition is invalid.");
            }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            if (objectPool == null)
            {
                throw new NWFrameworkException("Object pool is invalid.");
            }
            ...
            int priority) where T : ObjectBase
        {
            ...
            {
                throw new NWFrameworkException(Utility.Text.Format("Already exist object pool '{0}'.", typeNamePair));
            }
