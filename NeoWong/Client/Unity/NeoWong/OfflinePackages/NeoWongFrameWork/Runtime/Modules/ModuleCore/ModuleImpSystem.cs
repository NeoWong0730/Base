...
namespace NWFramework
{
    ...
    public static class ModuleImpSystem
    {
        ...
        private const string ModuleRootNameSpace = "NWFramework.";

        private static readonly Dictionary<Type, ModuleImp> _moduleMaps = new Dictionary<Type, ModuleImp>(DesignModuleCount);
        private static readonly NWFrameworkLinkedList<ModuleImp> _modules = new NWFrameworkLinkedList<ModuleImp>();
        private static readonly NWFrameworkLinkedList<ModuleImp> _updateModules = new NWFrameworkLinkedList<ModuleImp>();   
        ...
        public static T GetModule<T>() where T : class
        {
            ...
            {
                throw new NWFrameworkException(Utility.Text.Format("You must get a Framework module, but '{0}' is not.", module.FullName));
            }
            ...
            if (moduleType == null)
            {
                ...
                if (moduleType == null)
                {
                    throw new NWFrameworkException(Utility.Text.Format("Can not find Game Framework module type '{0}'.", moduleName));
                }
            }
        ...
        private static ModuleImp CreateModule(Type moduleType)
        {
            ...
            if (moduleImp == null)
            {
                throw new NWFrameworkException(Utility.Text.Format("Can not create module '{0}'.", moduleType.FullName));
            }
