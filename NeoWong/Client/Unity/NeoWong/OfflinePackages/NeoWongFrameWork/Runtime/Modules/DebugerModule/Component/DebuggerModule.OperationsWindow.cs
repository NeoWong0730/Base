        ...
        private sealed class OperationsWindow : ScrollableDebuggerWindowBase
        {
            protected override void OnDrawScrollableWindow()
            {
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    if (GUILayout.Button("Shutdown NWFramework (None)", GUILayout.Height(30f)))
                    {
                        ModuleSystem.Shutdown(ShutdownType.None);
                    }
                    if (GUILayout.Button("Shutdown NWFramework (Restart)", GUILayout.Height(30f)))
                    {
                        ModuleSystem.Shutdown(ShutdownType.Restart);
                    }
                    if (GUILayout.Button("Shutdown NWFramework (Quit)", GUILayout.Height(30f)))
                    {
