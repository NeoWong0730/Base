    ...
    internal sealed partial class DebuggerManager : ModuleImp, IDebuggerManager
    {
        ...
        private sealed class DebuggerWindowGroup : IDebuggerWindowGroup
        {
            ...
            public void RegisterDebuggerWindow(string path, IDebuggerWindow debuggerWindow)
            {
                if (string.IsNullOrEmpty(path))
                {
                    throw new NWFrameworkException("Path is invalid.");
                }
                ...
                if (pos < 0 || pos >= path.Length - 1)
                {
                    ...
                    {
                        throw new NWFrameworkException("Debugger window has been registered.");
                    }
                ...
                else
                {
                    ...
                    if (debuggerWindowGroup == null)
                    {
                        ...
                        {
                            throw new NWFrameworkException("Debugger window has been registered, can not create debugger window group.");
                        }
