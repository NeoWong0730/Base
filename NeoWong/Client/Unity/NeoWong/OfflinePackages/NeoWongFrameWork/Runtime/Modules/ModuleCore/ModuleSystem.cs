...
namespace NWFramework
{
    ...
    {
        private static readonly NWFrameworkLinkedList<Module> _modules = new NWFrameworkLinkedList<Module>();

        /// <summary>
        /// 游戏框架所在的场景编号
        /// </summary>
        internal const int NWFrameworkSceneId = 0;
        ...
        {
            Log.Info("Shutdown NWFramework ({0})...", shutdownType);
            ...
            {
                SceneManager.LoadScene(NWFrameworkSceneId);
                return;
            }
        ...
        internal static void RegisterModule(Module module)
        {
            ...
            while (current != null)
            {
                if (current.Value.GetType() == type)
                {
                    Log.Error("NWFramework component type '{0}' is already exist.", type.FullName);
                    return;
                }
